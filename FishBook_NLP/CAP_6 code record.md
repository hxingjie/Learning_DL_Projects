...
class RNNlm(torch.nn.Module):
    def __init__(self, vocab_size, hidden_size):
        ...
        self.lstm = torch.nn.LSTM(input_size=hidden_size, hidden_size=hidden_size, num_layers=1)
    ...
    def forward(self, inputs):
        ...
        outputs, _ = self.lstm(in_embed)  # outputs: (Seq, B, H)
...
# Epoch=249, Epoch ppl=195.15651432188517
```
...
class RNNlm(torch.nn.Module):
    def __init__(self, vocab_size, hidden_size):
        ...
        self.lstm = torch.nn.LSTM(input_size=hidden_size, hidden_size=hidden_size, num_layers=2, dropout=DROPOUT_RATIO)  # add lstm's layers and use dropout

        self.fc = torch.nn.Linear(in_features=hidden_size, out_features=vocab_size)
        self.fc.weight.data = self.embed.weight.data  # share weights
...
for epoch in range(MAX_EPOCH):
    ...
    scheduler.step()  # dynamically adjust learning rate
    list_loss.append(np.exp(epoch_loss_sum / len(train_loader)))
    print(f"Epoch={epoch}, Epoch ppl={np.exp(epoch_loss_sum / len(train_loader))}")
